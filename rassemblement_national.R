...
tableau_articles <- data.frame(titre=NA,url_texte=NA,texte=NA,categorie=NA,date=NA,auteur=NA)[0,]

# Lister les url à parcourir
list_url <- paste("https://rassemblementnational.fr/actualites/page/",c(1:nb_pages),"/",sep="")

# Correction spéciale pour la page 1
list_url[1] <- "https://rassemblementnational.fr/actualites/"
...
for (url in list_url){
  ...
  if(class(page)[1] != "try-error"){
    ...
    # Lister les articles
    liste_actu <- getNodeSet(page_html,"//article")
    ...
    for (id_actu in 1:length(liste_actu)){
      ...
      titre <- xmlValue(getNodeSet(actu,".//h2")[[1]])
      # Nettoyer l'info
      titre <- str_remove_all(titre,"\n|\t")
      url_texte <- xmlAttrs(getNodeSet(actu,".//h2/a")[[1]])[['href']]
      resume <- xmlValue(getNodeSet(actu,".//div[@class='fusion-post-content-container']")[[1]])
      
      # Attention : il n'y a parfois pas de categorie
      categorie <- try(xmlValue(getNodeSet(actu,".//a[@rel='category tag']")[[1]]),silent=TRUE)
      if(class(categorie) == "try-error") categorie <- NA
      
      auteur <- xmlValue(getNodeSet(actu,".//a[@rel='author']")[[1]])
      
      date <- xmlValue(getNodeSet(actu,".//div[@class = 'fusion-meta-info']")[[1]])
      ...
      if(class(page_texte)[1] != "try-error"){
        ...
        # Extraire les données
        texte <- xmlValue(getNodeSet(page_texte,"//div[@class='post-content']")[[1]])
        
        # Insérer une nouvelle ligne dans le tableau
        tableau_articles[nrow(tableau_articles)+1,] <-  list(titre,url_texte,texte,categorie,date,auteur)
      } else {
...
# En format R
save(tableau_articles,file = 'FOLDER/tableau_articles_RN')

# En format CSV
write.csv2(tableau_articles,file = 'FOLDER/tableau_articles_RN.csv')